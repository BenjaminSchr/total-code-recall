...
### Attempt 1
- Date: 2026-05-23
- Result: Created `skills/code-onboard/SKILL.md` with all 8 steps covering the full onboard pipeline. Each step includes either a shell command or a complete inline Python script body for Claude to write to /tmp/ and execute. Steps: Git Gate, Check Prerequisites, Create Project Table, Discover Files, Chunk Files, Generate Summaries + Embeddings (with batch INSERT), Update _index_meta (UPSERT), and Report. Config section lists all 6 env vars with defaults. Error Handling section covers timeouts, model pulls, DB failures, re-indexing, and embedding dimension mismatches. Verify: 11 "Step" references found (required 8+).
