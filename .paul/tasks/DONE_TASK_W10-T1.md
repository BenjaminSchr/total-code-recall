...
### Attempt 1
- Date: 2026-05-23
- Result: Added 3-layer config loader (`_GLOBAL_CONFIG`, `_CONFIG_PATH`, `_cfg()`) to the main Config section of all 5 SKILL.md files. Each file got `import json` added and the `os.getenv()` variable assignments replaced with `_cfg()` calls. Only variables present in each file's original config section were replaced (tcr-onboard and tcr-update got all 6 vars; tcr-search and tcr-explain got 3 vars; tcr-overview got 2 vars). No secondary embedded scripts or SQL/embedding logic was touched.
- Files Changed: skills/tcr-onboard/SKILL.md, skills/tcr-update/SKILL.md, skills/tcr-search/SKILL.md, skills/tcr-overview/SKILL.md, skills/tcr-explain/SKILL.md
- Issues: none
- Status: DONE
