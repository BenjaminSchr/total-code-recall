...
### Attempt 1
- Date: 2026-05-23
- Result: Added .env loader block to every Python script block containing os.getenv in all 3 SKILL.md files. code-onboard: 5 blocks (Config, tcr_check_db.py, tcr_create_table.py, tcr_index.py, tcr_update_meta.py). code-update: 5 blocks (Config, tcr_check_meta.py, tcr_delete_stale.py, tcr_index.py, tcr_update_meta.py). code-search: 4 blocks (Config, tcr_check_search_meta.py, tcr_embed_query.py, tcr_search.py). Loader placed after imports and before any os.getenv call in each block.
- Files Changed: skills/code-onboard/SKILL.md, skills/code-update/SKILL.md, skills/code-search/SKILL.md
- Issues: none
- Status: DONE
