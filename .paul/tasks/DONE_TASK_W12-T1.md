...
### Attempt 1
- Date: 2026-05-23
- Result: Replaced the prose-only Toggle 2 section with functional Python code that asks Ollama vs OpenRouter, handles OpenRouter API key reuse, shows common embedding model choices, and writes `embedding_provider` and `embedding_model` to config. Also added explicit Python code block in the Change Mode section for handling `[2]` Embedding Provider, mirroring the Toggle 2 logic. Updated `[2]` prose line to match. Verify check: `grep -c "embedding_provider" skills/tcr-config/SKILL.md` returned 7 (>= 2 required).
