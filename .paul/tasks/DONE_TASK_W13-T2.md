...
### Attempt 1
- Date: 2026-05-23
- Result: Added `DB_PROVIDER = _cfg("DB_PROVIDER", "db_provider", "local")` to config section of both tcr-onboard and tcr-update SKILL.md files. Audited all embedded Python scripts for COPY statements — none found. All SQL uses parameterized INSERT/SELECT which are fully compatible with Supabase pooler (session mode). Vector distance `<=>` operator is natively supported in Supabase pgvector. SSL is handled via sslmode=require in the connection URL from tcr-config.
- Files Changed: skills/tcr-onboard/SKILL.md, skills/tcr-update/SKILL.md
- Issues: none — no COPY statements found, SQL is Supabase-compatible as-is
- Status: DONE
