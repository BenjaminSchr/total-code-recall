...
### Attempt 1
- Date: 2026-05-23
- Result: Added `_call_openrouter_with_retry()` with exponential backoff on 429 to both SKILL.md files. Refactored the sequential chunk loop in both scripts to pre-fetch all summaries first: OpenRouter path uses `ThreadPoolExecutor(max_workers=PARALLEL_WORKERS)` with `as_completed` for parallel fetching; Ollama path remains sequential. The embedding + insert loop is unchanged and still runs sequentially after all summaries are collected.
