...
### Attempt 1
- Date: 2026-05-23
- Result: Added file list export after Step 4 (exports only paths via `[fp for fp, _ in files_to_index]`). Added Step 5b with complete inline tcr_parse_ast.py script: tree-sitter import check (non-blocking exit 0 on ImportError), reads /tmp/tcr_files.json, skips non-.py files with message, two-pass approach (Pass 1: insert entities with RETURNING to build name->id map; Pass 2: extract calls/imports relations), clears existing entities before insert (CASCADE handles relations), uses `call` node type (tree-sitter-python correct type, not `call_expression`), outputs AST_OK/AST_SKIP.
