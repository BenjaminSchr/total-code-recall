...
### Attempt 1
- Date: 2026-05-23
- Result: Added `{PROJECT_NAME}_summaries` table DDL (CREATE TABLE + 2 indexes) to `CREATE_TABLE_SQL` in Step 3 of SKILL.md, after the relations table. Added `DELETE FROM {PROJECT_NAME}_summaries` cleanup line in `tcr_index.py` alongside existing `DELETE FROM {PROJECT_NAME}`. Added summaries template comment block to `setup_db.sql`.
